...
class Model(ModelBase):
  ...
  def _build_latent_network(self,
                            ...
                            proba_h_sparse_loss_weight=0.0,
                            proba_h_decov_loss_weight=0.0):
    """Builds the Multiple Instance Detection Network.
    ...
    if proba_h_use_sigmoid:
      sparse_loss = tf.multiply(
          tf.reduce_mean(proba_h_given_c), proba_h_sparse_loss_weight)
      if proba_h_sparse_loss_weight > 0:
...
#      #   logits_r_given_h shape = [batch, max_num_proposals, num_latent_factors].
#      #   logits_h_given_c shape = [num_latent_factors, num_classes].
#      logits_r_given_h = tf.expand_dims(logits_r_given_h, axis=-1)
#      logits_r_given_c = tf.reduce_max(
#          tf.multiply(
#              logits_r_given_h,
#              tf.expand_dims(tf.expand_dims(proba_h_given_c, axis=0),
#                             axis=0)),
#          axis=2)
#    else:
      ...
      if options.attention_type == nod2_model_pb2.NOD2Model.PER_CLASS:
        ...
        (midn_class_logits, midn_proposal_scores, midn_proba_r_given_c,
         midn_proba_h_given_c) = self._build_latent_network(
             ...
             proba_h_use_sigmoid=options.
             per_hidden_class_prior_proba_use_sigmoid,
             proba_h_sparse_loss_weight=options.
             per_hidden_class_prior_proba_sparse_loss_weight,
             proba_h_decov_loss_weight=options.
             per_hidden_class_prior_proba_decov_loss_weight)
      else:
  ...
  def build_prediction(self, examples, **kwargs):
    ...
    if is_training or len(options.eval_max_dimension) == 0:
  ...
  def build_loss(self, predictions, examples, **kwargs):
    ...
    with tf.name_scope('losses'):
      ...
      if options.midn_loss_negative_mining == nod2_model_pb2.NOD2Model.HARDEST:
      ...
      elif options.midn_loss_negative_mining == nod2_model_pb2.NOD2Model.NONE:
