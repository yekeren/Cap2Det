...
from core.standard_fields import NOD4Predictions
...
class Model(ModelBase):
  ...
  def __init__(self, model_proto, is_training=False):
    ...
    Args:
      model_proto: an instance of nod4_model_pb2.NOD4Model
      is_training: if True, training graph will be built.
    """
    super(Model, self).__init__(model_proto, is_training)

    if not isinstance(model_proto, nod4_model_pb2.NOD4Model):
      raise ValueError('The model_proto has to be an instance of NOD4Model.')
  ...
  def _post_process(self, inputs, predictions):
    ...
    for i in range(1 + options.oicr_iterations):
      ...
      proposal_scores = predictions[NOD4Predictions.oicr_proposal_scores +
                                    '_at_{}'.format(i)]
  ...
  def _build_prediction(self, examples, post_process=True):
    ...
    with slim.arg_scope(build_hyperparams(options.fc_hyperparams, is_training)):
      for i in range(options.oicr_iterations):
        predictions[NOD4Predictions.oicr_proposal_scores + '_at_{}'.format(
    ...
    with slim.arg_scope(build_hyperparams(options.fc_hyperparams, is_training)):
      if options.attention_type == nod4_model_pb2.NOD4Model.PER_CLASS:
      ...
      elif options.attention_type == nod4_model_pb2.NOD4Model.PER_CLASS_TANH:
        (midn_class_logits, midn_proposal_scores,
    ...
    predictions.update({
        ...
        proposals,
        NOD4Predictions.midn_class_logits:
        midn_class_logits,
  ...
  def build_prediction(self, examples, **kwargs):
    ...
    for min_dimension in options.eval_min_dimension:
      ...
      for i in range(1 + options.oicr_iterations):
        proposals_scores = predictions[NOD4Predictions.oicr_proposal_scores +
                                       '_at_{}'.format(i)]
    ...
    for i in range(1 + options.oicr_iterations):
      ...
      predictions_aggregated[NOD4Predictions.oicr_proposal_scores +
  ...
  def build_loss(self, predictions, examples, **kwargs):
    ...
    with tf.name_scope('losses'):
      ...
      if options.label_strategem == nod4_model_pb2.NOD4Model.EXACTLY_MATCH:
        ...
        labels_ps = tf.where(select_op, tf.zeros_like(labels_ps), labels_ps)
      ...
      proposal_scores_0 = predictions[NOD4Predictions.oicr_proposal_scores +
                                      '_at_0']
      if options.oicr_use_proba_r_given_c:
        proposal_scores_0 = predictions[NOD4Predictions.midn_proba_r_given_c]
      ...
      for i in range(options.oicr_iterations):
        proposal_scores_1 = predictions[NOD4Predictions.oicr_proposal_scores +
                                        '_at_{}'.format(i + 1)]
