...
class UtilsTest(tf.test.TestCase):
  ...
  def test_gather_in_batch_captions(self):
    ...
    with self.test_session(graph=g) as sess:
      ...
      (image_ids_value, caption_strings_value, caption_lengths_value) = sess.run( 
          [image_ids_gathered, caption_strings_gathered, caption_lengths_gathered], 
          feed_dict={ 
            image_id: ["a", "b"], 
            num_captions: [3, 1],
            caption_strings: [
